...
import { Calendar, User, ArrowRight, Search, Tag, Sparkles, MessageSquare } from "lucide-react";
...
export async function generateMetadata({
  ...
  const title = isJa
    ? "リーダーシップ・ソーツ | 日印ビジネスビューロー"
    : "Leadership Thoughts | JIBB — Japan India Business Bureau";
  const description = isJa
    ? "JIBBの共同創設者とリーダーシップチームによる、日印ビジネスの展望と戦略的見解。"
    : "Articles and perspectives from JIBB's co-founders and leadership on Japan-India bilateral business, policy, and innovation.";

  const canonicalUrl = `${SITE_URL}/${locale}/resources/leadership-thoughts`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: `${SITE_URL}/en/resources/leadership-thoughts`,
        ja: `${SITE_URL}/ja/resources/leadership-thoughts`,
        "x-default": `${SITE_URL}/en/resources/leadership-thoughts`,
      },
    },
...
export default async function LeadershipThoughtsPage({ params, searchParams }: PageProps) {
  const { locale } = await params;
  ...
  const allPosts = await getAllPosts("leadership-thoughts", locale);
        ...
        <div className="section-container flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link href="/resources/leadership-thoughts">
              ...
              >
                {locale === "ja" ? "すべて" : "All Perspectives"}
              </Badge>
            </Link>
            {allTags.map((tag) => (
              <Link key={tag} href={`/resources/leadership-thoughts?tag=${encodeURIComponent(tag)}&q=${search}`}>
                <Badge
          ...
          <form method="GET" action="" className="relative w-full md:w-80">
            ...
            <input
              ...
              name="q"
              defaultValue={search}
              placeholder={locale === "ja" ? "記事を検索..." : "Search articles..."}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2">
                    <div className="relative h-64 sm:h-96 lg:h-full min-h-[300px] overflow-hidden">
                      ...
                      <Badge className="absolute top-6 left-6 bg-jibb-orange text-white hover:bg-jibb-orange font-bold uppercase tracking-wider text-[10px]">
                        {locale === "ja" ? "注目オピニオン" : "Featured Vision"}
                      </Badge>
                    </div>
                    ...
                    <div className="p-8 sm:p-12 flex flex-col justify-center space-y-6">
                      ...
                      <h2 className="text-2xl sm:text-3xl font-extrabold text-foreground tracking-tight leading-tight group-hover:text-jibb-orange transition-colors">
                        <Link href={`/resources/leadership-thoughts/${featuredPost.slug}`}>
                          {featuredPost.title}
                        </Link>
                      </h2>
                      ...
                      <div className="border-t border-border/60 pt-6 flex items-center justify-between">
                        ...
                        <Link href={`/resources/leadership-thoughts/${featuredPost.slug}`}>
                          <Button variant="link" className="text-jibb-orange p-0 font-bold gap-1 group-hover:gap-2 transition-all">
                            {locale === "ja" ? "続きを読む" : "Read Vision"} <ArrowRight className="size-4" />
                          </Button>
                        </Link>
                      </div>
                    </div>
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                      <div className="space-y-3">
                        ...
                        <h3 className="text-lg font-bold text-foreground tracking-tight leading-snug group-hover:text-jibb-orange transition-colors line-clamp-2">
                          <Link href={`/resources/leadership-thoughts/${post.slug}`}>
                            {post.title}
                          </Link>
                        </h3>
