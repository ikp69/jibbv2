...
export default function EventsError({
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-6 space-y-4">
      ...
      <div>
        <h2 className="text-lg font-bold text-slate-900">Failed to load networking events</h2>
        <p className="text-sm text-slate-500 mt-1">{error.message || "An unexpected error occurred."}</p>
      </div>
      <button
        onClick={() => reset()}
        className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg shadow-md cursor-pointer transition-colors"
      >
