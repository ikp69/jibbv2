...
const programRowVariant = {
  hidden: { opacity: 0 },
  visible: (i: number) => ({
    opacity: 1, transition: { duration: 0.5, delay: i * 0.05, ease: [0.33, 1, 0.68, 1] as const }
  })
}
const venueRowVariant = {
  hidden: { opacity: 0, x: -30 },
  visible: (i: number) => ({
    opacity: 1, x: 0, transition: { duration: 0.5, delay: i * 0.12, ease: [0.33, 1, 0.68, 1] as const }
  })
}
...
const getPartnerLogo = (name: string): string | null => {
  ...
  if (n.includes('iesa') || n.includes('electronics & semiconductor')) return '/jisc/govt-backed/iesa-logo.webp'
  if (n.includes('jspe') || n.includes('precision engineering') || n.includes('cmp')) return '/jisc/jspe-logo.png'
  if (n.includes('jisc') || n.includes('semiconductor committee')) return '/jisc/jisc-india-japan.png'
  return null
}
...
export default function SemiconEventPage() {
  ...
  const labels = {
    en: { backToEvents: '← All Events', programTitle: 'Program Schedule', organizersTitle: 'Organizers & Partners', venueTitle: 'Venue & Access', time: 'Time', reception: 'Reception', capacity: 'Capacity', openMaps: 'Open in Google Maps', register: 'Register Now', viewProgram: 'View Program', organizer: 'Organizer', coOrganizers: 'Co-organizers', specialSupport: 'Special Support', supporters: 'Supporters', specialCoop: 'Special Cooperation', travelDesc: 'Distance between central stations and airports in major Japanese cities', contactTitle: 'Contact Information', contactOrg: 'NPO Japan India Business Bureau (JIBB) / Japan India Consulting', contactEmail: 'trade@ji-consulting.jp' },
    ja: { backToEvents: '← イベント一覧', programTitle: 'プログラム', organizersTitle: '主催・後援', venueTitle: '会場・アクセス', time: '時間', reception: '懇親会', capacity: '定員', openMaps: 'Google マップで開く', register: '参加申込', viewProgram: 'プログラム', organizer: '主催', coOrganizers: '共催', specialSupport: '特別後援', supporters: '後援', specialCoop: '特別協力', travelDesc: '国内の主要都市における中心駅と空港との距離', contactTitle: 'お問い合わせ', contactOrg: 'NPO法人 日本インドビジネスビューロー（JIBB）/ 日印コンサルティング株式会社', contactEmail: 'trade@ji-consulting.jp' }
  }
  ...
  return (
      ...
      <div className="event-detail-poster-hero">
        <Image
          src={eventData.posterEn}
          alt="Semicon India 2026 Seminar"
          fill
          ...
          <div className="events-details">
            ...
            <div className="events-detail-item">
              ...
              <div>
                ...
                <p>{t.seminarCapacity}{t.receptionCapacity && ` | ${lb.reception}: ${t.receptionCapacity}`}</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="events-venue-split">
          <div className="events-venue-details">
            {[
              ...
              { icon: 'train', label: locale === 'ja' ? 'アクセス' : 'Access', value: locale === 'ja' ? '最寄り駅から徒歩圏内' : 'Walking distance from nearest station' },
